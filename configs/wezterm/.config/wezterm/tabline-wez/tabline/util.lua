...
local function require_component(object, v)
	...
	if object.tab_id then
		component = 'tabline-wez.tabline.components.tab.' .. v
	else
		component = 'tabline-wez.tabline.components.window.' .. v
	end
	return component
end

function M.extract_components(components_opts, attributes, object, format)
	local component_opts = require('tabline-wez.tabline.config').component_opts
	local components = {}
